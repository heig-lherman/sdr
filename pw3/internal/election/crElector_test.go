...
type mockRingManager struct {
	...
	sentMessages        chan dispatcher.Message
	simulatedReceptions chan dispatcher.Message
}

func (m mockRingManager) SendToNext(msg dispatcher.Message) {
	m.sentMessages <- msg
}

func (m mockRingManager) ReceiveFromPrev() dispatcher.Message {
	return <-m.simulatedReceptions
}

func (m mockRingManager) expectSentMessage(msg dispatcher.Message) {
...
func (m mockRingManager) expectNothingFor(duration time.Duration) {
	select {
...
func (m mockRingManager) simulateReception(msg dispatcher.Message) {
	m.simulatedReceptions <- msg
}

func NewMockRingManager(t *testing.T) mockRingManager {
	return mockRingManager{
		t:                   t,
		sentMessages:        make(chan dispatcher.Message, 100),
		simulatedReceptions: make(chan dispatcher.Message, 100),
	}
}
