...
func newRequestMessage(seqnum seqnum, payload []byte) *message {
	return &message{Type: reqMsg, Seqnum: seqnum, Payload: payload}
}

// newResponseMessage creates a new response message
func newResponseMessage(seqnum seqnum, payload []byte) *message {
	return &message{Type: rspMsg, Seqnum: seqnum, Payload: payload}
}
